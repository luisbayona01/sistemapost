                    ...
                    <tbody>
                        @foreach ($users as $item)
                            <tr class="hover:bg-gray-50">
                                <td class="text-gray-900 p-3 border border-gray-300">{{$item->empleado->razon_social ?? 'N/A'}}
                                </td>
                                <td class="text-gray-900 p-3 border border-gray-300">{{$item->name}}</td>
                                <td class="text-gray-900 p-3 border border-gray-300">{{$item->username}}</td>
                                ...
                                <td class="text-gray-900 p-3 border border-gray-300">
                                    <div class="flex justify-between items-center gap-2">
                                        <div>
                                            <button title="Opciones" class="text-gray-600 hover:text-gray-900 relative"
                                                onclick="document.getElementById('menu-{{$item->id}}').classList.toggle('hidden')">
                                                <i class="fas fa-ellipsis-v"></i>
                                            </button>
                                            <div id="menu-{{$item->id}}"
                                                class="hidden absolute bg-white border border-gray-300 rounded shadow-lg mt-1 z-10">
                                                <!-----Editar usuarios--->
                                                @can('editar-user')
                                ...
                                <div class="bg-white rounded-lg shadow-lg max-w-md w-full mx-4">
                                    ...
                                    <div class="bg-gray-100 border-t border-gray-300 px-6 py-4 flex justify-end gap-2">
                                        ...
                                        <form action="{{ route('users.destroy', ['user' => $item->id]) }}" method="post"
                                            ...
                                            @csrf
                                            <button type="submit"
                                                class="bg-red-600 hover:bg-red-700 text-white font-semibold py-2 px-6 rounded-lg transition-colors">Confirmar</button>
                                        </form>
                                    </div>
                                </div>
                            </div>
