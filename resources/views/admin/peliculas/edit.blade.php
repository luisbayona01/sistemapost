...
@section('content')
    <div class="w-full px-4 md:px-6 py-4">
        <h1 class="text-3xl font-bold text-center text-gray-900 mb-6">Editar Película</h1>
        ...
        <x-breadcrumb.template>
            ...
            <x-breadcrumb.item active='true' content="Editar: {{ $pelicula->titulo }}" />
        </x-breadcrumb.template>
            ...
            <form action="{{ route('peliculas.update', $pelicula) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
                        ...
                        <div class="lg:col-span-1 space-y-4">
                            ...
                            <div class="bg-gray-50 border-2 border-dashed border-gray-300 rounded-xl p-4 flex flex-col items-center justify-center text-center cursor-pointer hover:bg-gray-100 transition-colors relative h-[400px]"
                                ...
                                <div id="poster-placeholder" class="space-y-2 {{ $pelicula->afiche ? 'hidden' : '' }}">
                                ...
                                <img id="poster-preview"
                                    class="poster-preview {{ $pelicula->afiche ? '' : 'hidden' }} absolute inset-0 w-full h-full object-contain p-2"
                                    src="{{ $pelicula->afiche ? Storage::url($pelicula->afiche) : '' }}" alt="Vista previa">
                            </div>
                            ...
                            <div class="mt-4">
                                ...
                                <div class="relative">
                                    ...
                                    <input type="url" name="trailer_url" id="trailer_url"
                                        value="{{ old('trailer_url', $pelicula->trailer_url) }}"
                        ...
                        <div class="lg:col-span-2 space-y-6">
                            ...
                            <div class="flex justify-between items-start gap-4">
                                <!-- Título -->
                                <div class="flex-grow">
                                    <label for="titulo" class="block text-sm font-bold text-gray-700 mb-2">Título de la
                                        Película <span class="text-red-500">*</span></label>
                                ...
                                <div class="w-32">
                                    ...
                                    <select name="activo"
                                        class="w-full px-3 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                                        <option value="1" {{ $pelicula->activo ? 'selected' : '' }}>Activa</option>
                                        <option value="0" {{ !$pelicula->activo ? 'selected' : '' }}>Inactiva</option>
                                    </select>
                                </div>
                            </div>
                                ...
                                <div>
                                    ...
                                    <select name="genero" id="genero"
                                        ...
                                        @foreach(['Acción', 'Aventura', 'Comedia', 'Drama', 'Terror', 'Ciencia Ficción', 'Fantasía', 'Animación', 'Documental', 'Thriller', 'Romance'] as $gen)
                                            <option value="{{ $gen }}" {{ old('genero', $pelicula->genero) == $gen ? 'selected' : '' }}>{{ $gen }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <select name="clasificacion" id="clasificacion"
                                        ...
                                        required>
                                        <option value="">Seleccione...</option>
                                        <option value="ATP" {{ old('clasificacion', $pelicula->clasificacion) == 'ATP' ? 'selected' : '' }}>ATP (Apta Todo Público)</option>
                                        <option value="+7" {{ old('clasificacion', $pelicula->clasificacion) == '+7' ? 'selected' : '' }}>+7 Años</option>
                                        <option value="+13" {{ old('clasificacion', $pelicula->clasificacion) == '+13' ? 'selected' : '' }}>+13 Años</option>
                                        <option value="+16" {{ old('clasificacion', $pelicula->clasificacion) == '+16' ? 'selected' : '' }}>+16 Años</option>
                                        <option value="+18" {{ old('clasificacion', $pelicula->clasificacion) == '+18' ? 'selected' : '' }}>+18 Años (Restringida)</option>
                                    </select>
                                ...
                                <div>
                                    ...
                                    <select name="distribuidor_id" id="distribuidor_id"
                                        ...
                                        @foreach($distribuidores as $distribuidor)
                                            <option value="{{ $distribuidor->id }}" {{ old('distribuidor_id', $pelicula->distribuidor_id) == $distribuidor->id ? 'selected' : '' }}>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <input type="date" name="fecha_estreno" id="fecha_estreno"
                                        value="{{ old('fecha_estreno', optional($pelicula->fecha_estreno)->format('Y-m-d')) }}"
                                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="date" name="fecha_fin_exhibicion" id="fecha_fin_exhibicion"
                                        value="{{ old('fecha_fin_exhibicion', optional($pelicula->fecha_fin_exhibicion)->format('Y-m-d')) }}"
                                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <textarea name="sinopsis" id="sinopsis" rows="4"
                                    ...
                                    placeholder="Escriba aquí la sinopsis de la película...">{{ old('sinopsis', $pelicula->sinopsis) }}</textarea>
                ...
                <div class="bg-gray-50 border-t border-gray-200 px-8 py-5 flex justify-end gap-3">
                    ...
                    <button type="submit"
                        ...
                        <i class="fas fa-save mr-2"></i> Actualizar Película
                    </button>
                </div>
            </form>
        </div>
