...
@section('content')
    <div class="px-6 py-8 max-w-4xl mx-auto">
        <div class="mb-8">
            <h1 class="text-3xl font-bold text-slate-900">Editar Distribuidor</h1>
            <p class="text-slate-600 mt-1">Actualice la información de {{ $distribuidor->nombre }}</p>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            <form action="{{ route('distribuidores.update', $distribuidor) }}" method="POST" class="p-8">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    <div class="col-span-1 md:col-span-2">
                        ...
                        <input type="text" name="nombre" id="nombre" value="{{ old('nombre', $distribuidor->nombre) }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="contacto" id="contacto"
                            value="{{ old('contacto', $distribuidor->contacto) }}"
                            class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 transition-all outline-none"
                    ...
                    <div>
                        ...
                        <input type="email" name="email" id="email" value="{{ old('email', $distribuidor->email) }}"
                            class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 transition-all outline-none"
                    ...
                    <div>
                        ...
                        <select name="activo" id="activo"
                            class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 transition-all outline-none">
                            <option value="1" {{ old('activo', $distribuidor->activo) == '1' ? 'selected' : '' }}>Activo
                            </option>
                            <option value="0" {{ old('activo', $distribuidor->activo) == '0' ? 'selected' : '' }}>Inactivo
                            </option>
                        </select>
                    </div>
                    ...
                    <div class="col-span-1 md:col-span-2">
                        ...
                        <textarea name="notas" id="notas" rows="3"
                            ...
                            placeholder="Información relevante, convenios, etc.">{{ old('notas', $distribuidor->notas) }}</textarea>
                    </div>
                </div>
