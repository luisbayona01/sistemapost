...
@section('content')
    <div class="w-full px-4 md:px-6 py-4">
        <h1 class="text-3xl font-black text-slate-900 mb-6 uppercase tracking-tighter">Editar Información del Producto /
            Snack</h1>
        ...
        <x-breadcrumb.template class="mb-6">
            ...
            <x-breadcrumb.item active='true' content="Editar Registro" />
    ...
    try {
        ...
        echo "Formato no compatible";
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <div class="space-y-6">
                                ...
                                <div class="flex items-center space-x-3 bg-white p-3 rounded-lg border border-slate-200">
                                    <div class="flex-shrink-0">
                                        <label class="relative inline-flex items-center cursor-pointer">
                                            <input type="checkbox" name="estado" value="1" class="sr-only peer" {{ old('estado', $producto->estado) ? 'checked' : '' }}>
                                            <div
                                    ...
                                    <div class="flex-grow">
                                        ...
                                        <p class="text-[10px] text-slate-500">Si está activo, aparecerá inmediatamente en
                                            ventas.</p>
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <select data-size="4" title="Seleccione una marca" data-live-search="true"
                                        ...
                                        @foreach ($marcas as $item)
                                            <option value="{{$item->id}}" {{$producto->marca_id == $item->id || old('marca_id') == $item->id ? 'selected' : '' }}>
                                                {{$item->nombre}}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div>
                                    ...
                                    <select data-size="4" title="Seleccione una presentación" data-live-search="true"
                                        ...
                                        @foreach ($presentaciones as $item)
                                            <option value="{{$item->id}}" {{$producto->presentacione_id == $item->id || old('presentacione_id') == $item->id ? 'selected' : '' }}>
                                                {{$item->nombre}}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div>
                                    ...
                                    <select data-size="4" title="Seleccione la categoría" data-live-search="true"
                                        ...
                                        @foreach ($categorias as $item)
                                            <option value="{{$item->id}}" {{ $producto->categoria_id == $item->id || old('categoria_id') == $item->id ? 'selected' : '' }}>
                                                {{$item->nombre}}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="border-t-2 border-slate-200 pt-6 mt-6 bg-emerald-50/30 p-4 rounded-xl">
                                    <h3 class="text-xl font-bold text-emerald-800 mb-4 flex items-center">
                                        <i class="fas fa-calculator mr-2 text-emerald-600"></i> Parámetros Financieros
                                    </h3>

                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            <label for="gasto_operativo_fijo"
                                                class="block text-sm font-semibold text-slate-700 mb-2">Gasto Operativo
                                                Fijo:</label>
                                            <input type="number" step="0.01" name="gasto_operativo_fijo"
                                                id="gasto_operativo_fijo"
                                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500"
                                                value="{{ old('gasto_operativo_fijo', $producto->gasto_operativo_fijo) }}">
                                        </div>

                                        <div>
                                            <label for="precio"
                                                class="block text-sm font-semibold text-slate-700 mb-2">Precio de
                                                Venta:</label>
                                            <input type="number" step="0.01" name="precio" id="precio"
                                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 font-bold"
                                                value="{{ old('precio', $producto->precio) }}">
                                        </div>

                                        <div>
                                            <label for="tipo_impuesto"
                                                class="block text-sm font-semibold text-slate-700 mb-2">Impuesto
                                                Aplicable:</label>
                                            <select name="tipo_impuesto" id="tipo_impuesto"
                                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500">
                                                <option value="EXENTO" {{ old('tipo_impuesto', $producto->tipo_impuesto) == 'EXENTO' ? 'selected' : '' }}>Exento</option>
                                                <option value="IVA" {{ old('tipo_impuesto', $producto->tipo_impuesto) == 'IVA' ? 'selected' : '' }}>IVA</option>
                                                <option value="IMPOCONSUMO" {{ old('tipo_impuesto', $producto->tipo_impuesto) == 'IMPOCONSUMO' ? 'selected' : '' }}>
                                                    Impoconsumo</option>
                                            </select>
                                        </div>

                                        <div>
                                            <label for="porcentaje_impuesto"
                                                class="block text-sm font-semibold text-slate-700 mb-2">% de
                                                Impuesto:</label>
                                            <input type="number" step="0.01" name="porcentaje_impuesto"
                                                id="porcentaje_impuesto"
                                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500"
                                                value="{{ old('porcentaje_impuesto', $producto->porcentaje_impuesto) }}">
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div>
                            ...
                            <img id="img-default" class="w-full"
                ...
                <div class="bg-gray-100 border-t border-gray-300 px-6 py-4 text-center flex justify-center gap-2">
                    ...
                    <button type="reset"
                        class="bg-gray-500 hover:bg-gray-600 text-white font-semibold py-2 px-6 rounded-lg transition-colors">Reiniciar</button>
                </div>
            </form>
        </div>
