...
<div class="w-full px-4 md:px-6 py-4">
    ...
    <div class="bg-white rounded-lg shadow mb-6">
        ...
        <div class="px-6 py-4 space-y-2">
            ...
            @endif
            <div class="text-sm text-gray-600">
                Saldo inicial: {{$caja->saldo_inicial}}</div>
            @if ($caja->saldo_final)
            <div class="text-sm text-gray-600">
            ...
            <a href="{{route('ventas.create')}}">
                <button type="button" class="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded-lg transition-colors mr-2">Nueva venta</button>
            </a>
            @endcan

            @can('crear-movimiento')
            <a href="{{route('movimientos.create',['caja_id' => $caja->id])}}">
                <button type="button" class="bg-green-600 hover:bg-green-700 text-white font-semibold py-2 px-4 rounded-lg transition-colors mr-2">Nuevo retiro</button>
            </a>
            @endcan

            @can('cerrar-caja')
            <button type="button" class="bg-red-600 hover:bg-red-700 text-white font-semibold py-2 px-4 rounded-lg transition-colors"
    ...
    <div id="confirmModal-{{$caja->id}}" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
        <div class="bg-white rounded-lg shadow-lg max-w-md w-full mx-4">
            <div class="bg-gray-100 border-b border-gray-300 px-6 py-4 font-semibold text-gray-900 flex justify-between items-center">
                <h1 class="text-lg">Mensaje de confirmación</h1>
                <button type="button" class="text-gray-500 hover:text-gray-700" onclick="document.getElementById('confirmModal-{{$caja->id}}').classList.add('hidden')">
