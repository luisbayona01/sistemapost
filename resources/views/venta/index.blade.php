        ...
        @can('crear-venta')
            <div class="mb-6 flex gap-3">
                <a href="{{route('ventas.create')}}">
                    <button type="button"
                        class="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-6 rounded-lg transition-colors">Crear
                        venta</button>
                </a>
                <a href="{{ route('export.excel-ventas-all') }}">
                    <button type="button"
                        class="bg-green-600 hover:bg-green-700 text-white font-semibold py-2 px-6 rounded-lg transition-colors">Exportar
                        en excel</button>
                </a>
            </div>
        @endcan
                    ...
                    <tbody>
                        @foreach ($ventas as $item)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="border border-gray-300 p-3">
                                    <p class="font-semibold text-gray-900 mb-1">
                                        {{ ucfirst($item->cliente->persona->tipo_persona ?? 'N/A') }}
                                    </p>
                                    <p class="text-gray-600 text-sm mb-0">
                                        {{$item->cliente->persona->razon_social ?? 'Público General'}}
                                    </p>
                                </td>
                                <td class="border border-gray-300 p-3">
                                    <div class="w-32">
                                ...
                                <td class="border border-gray-300 p-3">
                                    <div class="flex gap-2 items-center justify-center flex-wrap">
                                        ...
                                        <a type="button"
                                            class="bg-gray-600 hover:bg-gray-700 text-white font-semibold py-1 px-3 rounded text-sm transition-colors"
                                        ...
                                        @can('eliminar-venta')
                                            <button type="button"
                                                ...
                                                onclick="confirmarAnulacion('{{$item->id}}')">
                                                Anular
                                            </button>
                                        @endcan
    ...
    <script>
        ...
        function confirmarAnulacion(idVenta) {
            Swal.fire({
                ...
                text: "Esta acción es irreversible y requiere supervisión.",
                icon: 'warning',
                input: 'password',
                inputLabel: 'PIN de Gerente / Root',
                inputPlaceholder: 'Ingrese su PIN de seguridad',
                showCancelButton: true,
                confirmButtonColor: '#d33',
                cancelButtonColor: '#3085d6',
                confirmButtonText: 'Autorizar Anulación',
                cancelButtonText: 'Cancelar',
                preConfirm: (pin) => {
                    if (!pin) {
                        Swal.showValidationMessage('Debe ingresar un PIN')
                    }
                    return pin;
                }
            }).then((result) => {
                if (result.isConfirmed) {
                    ...
                    form.action = "{{ route('ventas.destroy', ':id') }}".replace(':id', idVenta);
                    form.method = 'POST';
