...
@section('content')
    ...
    <form action="{{ route('ventas.store') }}" method="post">
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            ...
                            <div>
                                <label for="cliente_id" class="block text-sm font-medium text-gray-700 mb-2">
                                    Cliente:</label>
                                <select name="cliente_id" id="cliente_id" class="form-control selectpicker show-tick w-full"
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <select name="comprobante_id" id="comprobante_id"
                                        class="form-control selectpicker w-full" title="Selecciona">
                ...
                <div class="bg-white rounded-lg shadow">
                    ...
                    <div class="p-6 border-2 border-blue-600">
                        <div class="space-y-6">
                            ...
                            <!-----Producto---->
                            <div>
                                <select id="producto_id" class="form-control selectpicker w-full" data-live-search="true"
                                    data-size="1" title="Busque un producto aquí">
                                    @foreach ($productos as $item)
                                        <option
                                            value="{{$item->id}}-{{$item->cantidad}}-{{$item->precio}}-{{$item->nombre}}-{{$item->sigla}}">
                                ...
                                <div class="md:w-1/2">
                                    <div class="flex items-center gap-4">
                                        <label for="stock" class="text-sm font-medium text-gray-700 whitespace-nowrap">
                                            En stock:</label>
                                        <div class="flex-1">
                                            <input disabled id="stock" type="text"
                                ...
                                <div class="md:w-1/2">
                                    <div class="flex items-center gap-4">
                                        <label for="precio" class="text-sm font-medium text-gray-700 whitespace-nowrap">
                                            Precio:</label>
                                        <div class="flex-1">
                                            <input disabled id="precio" type="number"
                                                class="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-50"
                                                step="any">
                                        </div>
                                    </div>
                                ...
                                <table id="tabla_detalle" class="w-full border-collapse">
                                    <thead>
                                        <tr class="bg-blue-600 text-white">
                                            <th class="border border-gray-300 p-3 text-left text-sm font-semibold">Producto
                                            </th>
                                            <th class="border border-gray-300 p-3 text-left text-sm font-semibold">
                                                Presentación</th>
                                            <th class="border border-gray-300 p-3 text-center text-sm font-semibold">
                                                Cantidad</th>
                                            <th class="border border-gray-300 p-3 text-right text-sm font-semibold">Precio
                                            </th>
                                            <th class="border border-gray-300 p-3 text-right text-sm font-semibold">Subtotal
                                            </th>
                                            <th class="border border-gray-300 p-3 text-center text-sm font-semibold">Acción
                                            </th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                    ...
                                    <tfoot class="font-semibold bg-gray-100 border-t-2 border-gray-300">
                                        ...
                                        <tr class="bg-blue-50">
                                            <td colspan="4" class="border border-gray-300 p-3 text-right font-bold text-lg">
                                                Total</td>
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <input type="number" id="dinero_recibido" name="monto_recibido"
                                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                        step="any">
                                </div>
                                ...
                                <div>
                                    <label for="vuelto" class="block text-sm font-medium text-gray-700 mb-2">
                                        Vuelto:</label>
                                    <input readonly type="number" name="vuelto_entregado" id="vuelto"
                                        class="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-50" step="any">
                                </div>
                            </div>
        ...
        <div class="hidden fixed inset-0 z-50 overflow-y-auto" id="exampleModal" aria-labelledby="exampleModalLabel"
            aria-hidden="true">
            <div class="flex items-center justify-center min-h-screen">
                <div class="absolute inset-0 bg-gray-500 bg-opacity-75 transition-opacity"
                    onclick="document.getElementById('exampleModal').classList.add('hidden')"></div>
                <div class="relative bg-white rounded-lg shadow-xl max-w-md w-full mx-4">
                    ...
                    <div class="flex gap-3 p-4 border-t border-gray-200">
                        <button type="button"
                            class="flex-1 px-4 py-2 bg-gray-300 hover:bg-gray-400 text-gray-900 rounded-lg transition-colors"
                            onclick="document.getElementById('exampleModal').classList.add('hidden')">Cerrar</button>
                        <button id="btnCancelarVenta" type="button"
                            class="flex-1 px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg transition-colors"
                            onclick="document.getElementById('exampleModal').classList.add('hidden')">Confirmar</button>
                    </div>
                </div>
            </div>
        </div>
...
@push('js')
    ...
    <script>
        ...
        // ============================================
        // OFFLINE MODE LOGIC (RESILIENCIA)
        // ============================================
        ...
        $('form').submit(function (e) {
            if (!navigator.onLine) {
                e.preventDefault();
                saveOfflineSale($(this));
            }
        });

        function saveOfflineSale(form) {
            // Serializar datos
            let formData = form.serializeArray();
            let saleData = {};
            $(formData).each(function (index, obj) {
                if (saleData[obj.name]) {
                    if (!Array.isArray(saleData[obj.name])) {
                        saleData[obj.name] = [saleData[obj.name]];
                    }
                    saleData[obj.name].push(obj.value);
                } else {
                    saleData[obj.name] = obj.value;
                }
            });

            // Agregar Timestamp y ID temporal
            saleData._offline_id = Date.now();
            saleData._created_at = new Date().toISOString();

            // Guardar en LocalStorage
            let pendingSales = JSON.parse(localStorage.getItem('offline_sales') || '[]');
            pendingSales.push(saleData);
            localStorage.setItem('offline_sales', JSON.stringify(pendingSales));

            // Limpiar interfaz (Simular éxito)
            cancelarVenta(); // Esta función ya limpia la tabla y campos
            Swal.fire({
                title: 'Venta Guardada (Offline)',
                text: 'Se sincronizará cuando vuelva internet.',
                icon: 'info',
                confirmButtonText: 'Ok'
            });
        }

        // 3. Sincronizar al volver online
        function syncOfflineSales() {
            let pendingSales = JSON.parse(localStorage.getItem('offline_sales') || '[]');

            if (pendingSales.length === 0) return;

            // Iterar y enviar (Secuencial para evitar saturación)
            // Nota: En producción idealmente se enviaría un batch

            let promises = pendingSales.map(sale => {
                return $.ajax({
            ...
            Promise.allSettled(promises).then((results) => {
                ...
                results.forEach((result, index) => {
                    if (result.status === 'fulfilled') {
                        ...
                        failed.push(pendingSales[index]);
                    }
                });

                if (failed.length === 0) {
                    localStorage.removeItem('offline_sales');
                    Swal.fire('Sincronización Completa', 'Todas las ventas offline se han subido.', 'success');
                } else {
                    // Mantener fallidos
                    localStorage.setItem('offline_sales', JSON.stringify(failed));
                    Swal.fire('Error de Sincronización', 'Algunas ventas no pudieron subirse. Se reintentará luego.', 'error');
                }
            });
        }

        // Intentar sincronizar al cargar si hay internet
        if (navigator.onLine) {
            syncOfflineSales();
        }
    </script>
